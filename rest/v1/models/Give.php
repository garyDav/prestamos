...
$app->get('/give/:pagina/:id',function($pagina,$id) use($app) {
	try {
		...
		$app->response->body(json_encode( array('id'=>$pagina,'error'=>$id) ));
...
$app->get('/give/users',function() use($app) {
	try {
		...
		$sql = "SELECT id id_pres,ci ci_pres,name name_pres,last_name last_name_pres  FROM user ORDER BY id_pres DESC LIMIT 4;";
...
$app->get('/give/clients',function() use($app) {
	try {
		...
		$sql = "SELECT id id_cli,ci ci_cli,name name_cli,last_name last_name_cli  FROM clients ORDER BY id_cli DESC LIMIT 4;";
		$result = $conex->prepare( $sql );
